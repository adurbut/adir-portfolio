...
const TONE_BG = { stone: 'var(--stone-2)', ink: 'var(--ink)', ink70: 'var(--ink-70)', muted: 'var(--muted)' };
const TONE_FG = { stone: 'var(--ink)', ink: 'var(--paper)', ink70: 'var(--paper)', muted: 'var(--paper)' };
  ...
  return (
    <nav className="nav">
      <div className="wrap nav-inner">
        <a className="nav-logo" href="#top"><img src="assets/adir-logo.svg" alt="Adir" /></a>
        ...
        <div className="nav-cta">
          <a className="btn btn-secondary btn-sm" href={STUDIO.wa} target="_blank" rel="noopener">וואטסאפ<Arr/></a>
        </div>
      </div>
    </nav>
  );
...
function Hero() {
  return (
    <header className="wrap p-hero" id="top">
      ...
      <p className="lead">עיצוב ובניית דפי נחיתה, אתרים, אפליקציות ומערכות — מהרעיון הראשוני ועד המוצר החי. כל פרויקט נבנה בקו נקי, מדויק, ובהתאמה אישית.</p>
      <div className="row-wrap">
        <a className="btn btn-primary" href="#work">לעבודות<Arr/></a>
        <a className="btn btn-ghost" href={STUDIO.wa} target="_blank" rel="noopener">בוא נדבר על פרויקט</a>
      </div>
  ...
  return (
    <article className="featured">
      <div className="f-body">
        <span className="f-index">{w.index} / 03 — מודגש</span>
        <span className="cat">{w.cat}</span>
        <h3>{w.title}</h3>
        <p>{w.desc}</p>
        <a className="ext" href={w.href} target="_blank" rel="noopener">לצפייה באתר החי<NW/></a>
      </div>
      <div className="slot-wrap" style={{ background: TONE_BG[w.tone] }}>
        <image-slot id={w.id} shape="rect" placeholder={'גרור צילום מסך של ' + w.title}></image-slot>
      </div>
    </article>
  );
}
  ...
  return (
    <article className="work-card">
      <div className="work-thumb" style={{ background: TONE_BG[w.tone] }}>
        <span className="num" style={{ color: w.tone === 'stone' ? 'var(--ink-70)' : 'var(--muted)' }}>{w.index} / 03</span>
        <image-slot id={w.id} shape="rect" placeholder={'גרור צילום מסך של ' + w.title}></image-slot>
      </div>
      <div className="work-body">
        ...
        <div className="work-foot">
          ...
          <a className="ext-min" href={w.href} target="_blank" rel="noopener">לצפייה באתר<NW/></a>
        </div>
      </div>
    </article>
  );
    ...
    <section className="section" id="work">
      <div className="wrap">
        <div className="section-head">
          ...
          <p className="section-note">מבחר עבודות חיות — מדף נחיתה ועד מערכת ניהול. גררו צילום מסך לכל כרטיס כדי להחליף את התצוגה.</p>
        </div>
        <div style={{ marginBottom: '26px' }}><Featured w={WORKS[0]} /></div>
        <div className="work-grid three">
          {WORKS.slice(1).map(w => <WorkCard key={w.id} w={w} />)}
        </div>
      </div>
    </section>
  );
...
function Services({ rich }) {
  return (
    <section className={'section' + (rich ? ' tint' : '')} id="services">
      <div className="wrap">
        <div className="section-head">
          <div>
          ...
          <p className="section-note">מקצה לקצה — אסטרטגיה, עיצוב ובנייה. עבודה צמודה, בקו אחד ונקי.</p>
        </div>
      ...
      <div className="wrap">
        <div className="contact-block">
          ...
          <div className="contact-actions">
            <a className="btn btn-wa" href={STUDIO.wa} target="_blank" rel="noopener">שליחת WhatsApp<Arr/></a>
    ...
    <footer className="footer">
      <div className="wrap">
        <div className="footer-top">
          <div>
            <div className="footer-logo" style={{ marginBottom: '18px' }}><img src="assets/adir-logo.svg" alt="Adir" /></div>
            <p className="muted" style={{ maxWidth: '30ch', margin: 0 }}>{STUDIO.name} — עיצוב ובניית חוויות דיגיטליות.</p>
          </div>
          <div className="footer-cols">
            <div className="footer-col">
              <h4>ניווט</h4>
              <a href="#work">עבודות</a><a href="#services">שירותים</a><a href="#contact">צור קשר</a>
            </div>
            <div className="footer-col">
              <h4>קשר</h4>
              <a href={STUDIO.wa} target="_blank" rel="noopener">WhatsApp</a>
...
function App() {
  ...
  return (
    <div className="adir-root">
      ...
      <Footer />
      <TweaksPanel>
        <TweakSection label="גוונים" />
        <TweakRadio label="עומק צבע" value={t.richness}
